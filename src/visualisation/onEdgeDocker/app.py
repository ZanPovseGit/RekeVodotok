...
pipe = os.path.join('model','preprocessor_pipeline.pkl')
znac = os.path.join('model','model.h5')
pret = os.path.join('model','modelRob.h5')

model_znacilni = tf.keras.models.load_model(znac)
model_pretok = tf.keras.models.load_model(pret)

preprocessor = joblib.load(pipe)
